...
There are three ways in which you can solve this problem as explained below.

A) Using CheckType.Fast
B) Creating IEObjectDescription with additional Data
C) Customizing the DefaultResourceDescriptionManager so that all DSL validations are called even if the EOD state remains unchanged.
