...
Above, user data is added while creating an EObjectDescription for the Clazz EObject's. In the example above, if you change the Customer Clazz from deprecated to non-deprecated or vice versa, the user-data changes. This changes the state of the EObjectDescription representing the Customer EObject.
